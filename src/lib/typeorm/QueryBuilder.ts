...
import { QueryFiltered, QuerySorted, UseQueryTypeOrm } from "./interface"
 ...
 * @returns
 */
export function _calculatePageSize(pageSize: string | number, maxLimit: number): number {
  const minLimit = 10
  ...
  if (!_.isEmpty(parsedFiltered)) {
    for (let i = 0; i < parsedFiltered.length; i += 1) {
      ...
      const check_query_like_mysql =
        ["mysql", "mariadb"].includes(String(options?.type)) && expect_numberic_or_uuid
